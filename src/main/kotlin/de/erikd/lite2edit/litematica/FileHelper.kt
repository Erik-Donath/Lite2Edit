package de.erikd.lite2edit.litematica

import net.kyori.adventure.nbt.BinaryTagIO
import net.kyori.adventure.nbt.CompoundBinaryTag
import java.io.BufferedInputStream
import java.io.IOException
import java.io.InputStream
import java.io.OutputStream
import java.io.PushbackInputStream
import java.util.zip.GZIPInputStream
import java.util.zip.GZIPOutputStream

object FileHelper {

    @Throws(IOException::class, IllegalArgumentException::class)
    fun loadLitematic(input: InputStream): CompoundBinaryTag {
        val buffered = BufferedInputStream(input, 64 * 1024)
        val pb       = PushbackInputStream(buffered, 2)

        // Detect GZIP by magic bytes 1F 8B
        val sig = ByteArray(2)
        val n   = pb.read(sig)
        if (n > 0) pb.unread(sig, 0, n)
        val isGzip = n == 2 && sig[0] == 0x1f.toByte() && sig[1] == 0x8b.toByte()

        val decoded: InputStream = if (isGzip) GZIPInputStream(pb) else pb

        val tag = BinaryTagIO.unlimitedReader().read(decoded)
        return tag as? CompoundBinaryTag
            ?: throw IllegalArgumentException("Root NBT tag is not a CompoundBinaryTag (got ${tag::class.java.simpleName})")
    }

    @Throws(IOException::class)
    fun saveLitematic(root: CompoundBinaryTag, output: OutputStream) {
        // Litematica files are always GZIP-compressed
        GZIPOutputStream(output).use { gzip ->
            BinaryTagIO.writer().write(root, gzip)
        }
    }
}